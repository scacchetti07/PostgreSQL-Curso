...
SELECT
	DISTINCT(LIV.NOME) AS LIVRO -- É necessário exibir os livros uma única vez, por isso a necessidade do distinct
FROM 
...
SELECT 
	ALN.NOME AS ALUNO,
	COUNT(EMP.IDEMPRESTIMO) AS TOTAL_EMPRESTIMOS
FROM 
	EMPRESTIMO EMP
LEFT OUTER JOIN
...
SELECT SUBSTRING(NOME FROM 5 FOR 6) AS AUTOR_5_TO_10 FROM AUTOR;
...
SELECT 
	EMP.DATA_EMPRESTIMO, 
	'R$ ' || ROUND(EMP.VALOR, 2) AS VALOR_TOTAL 
FROM 
	EMPRESTIMO EMP
WHERE 
	(SELECT 
		COUNT(EPL.IDEMPRESTIMO) 
	 FROM 
	 	EMPRESTIMO_LIVRO EPL 
	 WHERE 
	 	EPL.IDEMPRESTIMO = EMP.IDEMPRESTIMO) > 1;

-- 51. A data do empréstimo e o valor dos empréstimos que o valor seja menor que a soma de todos os empréstimos.
SELECT
	DATA_EMPRESTIMO,
	'R$ ' || ROUND(VALOR, 2) AS VALOR_TOTAL
FROM
	EMPRESTIMO
WHERE
	VALOR < (
